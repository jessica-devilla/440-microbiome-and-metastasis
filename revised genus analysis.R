...
# Calculate Spearman correlation coefficient between genus abundance and tumor stage
spearman_correlation <- apply(kraken_genusabundance[, -1], 2, function(x) {
  cor(x, kraken_genusabundance$pathologic_stage_label, method = "spearman")
})
...
weak_correlation_taxon <- names(which.min(abs(spearman_correlation)))
print(weak_correlation_taxon)
...
spear_maxabs_top5 <- head(abs(spearman_correlation), 5)
spear_maxabs_top5_taxa <- names(spear_maxabs_top5)
...
# Calculate summary statistics for each taxon within each tumor stage
#summary_stats <- lapply(unique(kraken_genusabundance$pathologic_stage_label), function(stage) {
  # Subset data for the current stage
 # stage_data <- kraken_genusabundance[kraken_genusabundance$pathologic_stage_label == stage, ]
  # Calculate summary statistics for each taxon
  #taxon_summary <- apply(stage_data[, -1], 2, function(x) {
   # c(mean = mean(x), sd = sd(x), median = median(x), IQR = IQR(x))
  })
  # Convert to dataframe and add stage label
  #taxon_summary <- as.data.frame(taxon_summary)
  #taxon_summary$Stage <- stage
